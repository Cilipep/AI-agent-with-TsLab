#!/usr/bin/env python3
"""Detailed Backtest: Moving M15 - SOL optimal params"""
...
def main():
    print("="*70)
    print("DETAILED BACKTEST: Moving M15 - SOL (optimal)")
    print("="*70)
    ...
    for i in range(warm,len(df)):
        ...
        sp=df['smma'].iloc[i-1]; ssp=df['smma_s'].iloc[i-1]; wp=df['wr'].iloc[i-1]; w8p=df['wr8'].iloc[i-1]
        if s>ss and sp<=ssp and w>w8 and wp<=w8p and w>w21: df.loc[df.index[i],'sig']=1
        elif s<ss and sp>=ssp and w<w8 and wp>=w8p and w<w21: df.loc[df.index[i],'sig']=-1
    
    sigs=int(df['sig'].abs().sum())
    print(f"Signals: {sigs}")
    
    # Backtest
    cap=CAP; pos=0; ent=0; ent_t=None; trades=[]; eq=[CAP]; eq_t=[df['ts'].iloc[0]]
    types=[]; dirs=[]; entries=[]; exits=[]; pnl_pcts=[]; entry_times=[]; exit_times=[]
    ...
    for i in range(1,len(df)):
        ...
        if pos==1:
            ...
            if lo<=slp:
                ...
                trades.append(pnl); dirs.append('LONG'); types.append('SL'); entries.append(ent); exits.append(slp)
                pnl_pcts.append(pnl); entry_times.append(ent_t); exit_times.append(ts)
                pos=0; eq.append(cap); eq_t.append(ts); continue
            elif hi>=tpp:
                pnl=((tpp-ent)/ent-COMM*2)*100; cap+=((tpp-ent)/ent-COMM*2)*cap
                trades.append(pnl); dirs.append('LONG'); types.append('TP'); entries.append(ent); exits.append(tpp)
                pnl_pcts.append(pnl); entry_times.append(ent_t); exit_times.append(ts)
                pos=0; eq.append(cap); eq_t.append(ts); continue
        elif pos==-1:
            slp=ent*(1+SL); tpp=ent*(1-TP)
            if hi>=slp:
                ...
                trades.append(pnl); dirs.append('SHORT'); types.append('SL'); entries.append(ent); exits.append(slp)
                pnl_pcts.append(pnl); entry_times.append(ent_t); exit_times.append(ts)
                pos=0; eq.append(cap); eq_t.append(ts); continue
            elif lo<=tpp:
                pnl=((ent-tpp)/ent-COMM*2)*100; cap+=((ent-tpp)/ent-COMM*2)*cap
                trades.append(pnl); dirs.append('SHORT'); types.append('TP'); entries.append(ent); exits.append(tpp)
                pnl_pcts.append(pnl); entry_times.append(ent_t); exit_times.append(ts)
                pos=0; eq.append(cap); eq_t.append(ts); continue
        ...
        if sg!=0:
            if pos==1 and sg==-1:
                ...
                trades.append(pnl); dirs.append('LONG'); types.append('EXIT'); entries.append(ent); exits.append(p)
                pnl_pcts.append(pnl); entry_times.append(ent_t); exit_times.append(ts); pos=0
            elif pos==-1 and sg==1:
                pnl=((ent-p)/ent-COMM*2)*100; cap+=((ent-p)/ent-COMM*2)*cap
                trades.append(pnl); dirs.append('SHORT'); types.append('EXIT'); entries.append(ent); exits.append(p)
                pnl_pcts.append(pnl); entry_times.append(ent_t); exit_times.append(ts); pos=0
    ...
    if pos!=0:
        ...
        entries.append(ent); exits.append(p); pnl_pcts.append(pnl)
        entry_times.append(ent_t); exit_times.append(df['ts'].iloc[-1])
    ...
    peak=eq[0]; mdd=0; mdd_idx=0
    for idx,v in enumerate(eq):
        if v>peak: peak=v
        dd=(peak-v)/peak*100
        if dd>mdd: mdd=dd; mdd_idx=idx
    ...
    for et,xt in zip(entry_times, exit_times):
        if et and xt: hold.append((xt-et).total_seconds()/3600)
    ...
    print("RESULTS")
    print("="*70)
    print(f"Period:              {df['ts'].iloc[0]} to {df['ts'].iloc[-1]}")
    ...
    # Save
    pd.DataFrame({'ts':entry_times,'dir':dirs,'type':types,'entry':entries,'exit':exits,'pnl':trades}).to_csv('tmp/moving_m15_sol_trades.csv',index=False)
    pd.DataFrame({'ts':eq_t,'equity':eq}).to_csv('tmp/moving_m15_sol_equity.csv',index=False)
    print("\nSaved to tmp/moving_m15_sol_trades.csv & equity.csv")
