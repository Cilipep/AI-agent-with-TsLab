#!/usr/bin/env python3
"""D1 analysis via H1 resampling"""

import pandas as pd
import numpy as np

INSTRUMENTS = ["SOL","LINK","ETH","DOGE","FET","INJ","BTC"]
...
def dl(sym):
    ...
    for f in [f"{sym}-USD", f"{sym}USD"]:
        try:
            t=yf.Ticker(f); d=t.history(period="2y", interval="1h")
            if not d.empty and len(d)>100:
                d=d.reset_index()[['Datetime','Open','High','Low','Close','Volume']]
                d.columns=['ts','o','h','l','c','v']; d['sym']=sym
                # Resample to D1
                d['ts']=pd.to_datetime(d['ts'])
                d.set_index('ts',inplace=True)
                d=d.resample('D').agg({'o':'first','h':'max','l':'min','c':'last','v':'sum'}).dropna()
                d=d.reset_index(); d['sym']=sym
                return d
        except: pass
    return None

def test(df,sl,tp,sp,wp):
    tmp=df.copy()
    ...
    for v in eq:
        ...
        dd=(peak-v)/peak*100
        if dd>mdd: mdd=dd
    return {'ret':ret,'mdd':mdd,'wr':len(wins)/len(trades)*100,'trades':len(trades),
            'sl':sl*100,'tp':tp*100,'smma':sp,'wrp':wp}

print("="*80)
print("MULTI-INSTRUMENT — D1 TIMEFRAME (from H1)")
print("="*80)
...
for sym in INSTRUMENTS:
    ...
    best=None
    for sl,tp,sp,wp in [(0.05,0.10,5,10),(0.05,0.15,7,10),(0.08,0.15,7,14),(0.10,0.20,10,14),(0.05,0.10,7,10)]:
        r=test(df,sl,tp,sp,wp)
