#!/usr/bin/env python3
"""Multi-instrument backtest on H1 timeframe"""

import pandas as pd
import numpy as np
from itertools import product

INSTRUMENTS = ["SOL","LINK","INJ","ETH","DOGE","FET","WIF","AVAX","BTC","BONK"]
...
def optimize(df, sl_range, tp_range, smma_range, wr_range):
    best=None
    for sl,tp,sp,wp in product(sl_range,tp_range,smma_range,wr_range):
        if tp<=sl: continue
        tmp=df.copy()
        ...
        if best is None or ret>best['ret']:
            best={'sl':sl*100,'tp':tp*100,'smma':sp,'wrp':wp,'trades':len(trades),
...
def main():
    print("="*80)
    print("MULTI-INSTRUMENT OPTIMIZATION — H1 TIMEFRAME")
    print("="*80)
    
    results=[]
    sl_r=[0.02,0.03,0.04]; tp_r=[0.04,0.06,0.08]; smma_r=[5,7]; wr_r=[10,14]
    
    for sym in INSTRUMENTS:
        print(f"{sym}...", end=" ")
        df=dl(sym)
        if df is None or df.empty: print("SKIP"); continue
        b=optimize(df,sl_r,tp_r,smma_r,wr_r)
        if b is None: print("no trades"); continue
        b['sym']=sym; results.append(b)
        print(f"Ret={b['ret']:+.2f}% WR={b['wr']:.0f}% MDD={b['mdd']:.1f}%")
    
    if not results: print("No results!"); return
    results.sort(key=lambda x: x['ret'], reverse=True)
    
    print("\n"+"="*80)
    print("H1 RESULTS (sorted by return)")
    print("="*80)
    print(f"{'Sym':<7} {'SL':>4} {'TP':>4} {'S':>2} {'WR':>2} {'#':>3} {'WR%':>5} {'Ret%':>7} {'MDD%':>6} {'AW%':>5} {'AL%':>5}")
    print("-"*80)
    for r in results:
        s="+" if r['ret']>0 else ""
        print(f"{r['sym']:<7} {r['sl']:.0f} {r['tp']:.0f} {r['smma']:>2} {r['wrp']:>2} "
              f"{r['trades']:>3} {r['wr']:>5.1f} {s}{r['ret']:>6.2f} {r['mdd']:>6.1f} {r['aw']:>5.2f} {r['al']:>5.2f}")
    
    prof=[r for r in results if r['ret']>0]
    print("-"*80)
    print(f"Profitable: {len(prof)}/{len(results)} | Avg Return: {np.mean([r['ret'] for r in results]):.2f}%")
    
    pd.DataFrame(results).to_csv('tmp/moving_m15_h1_results.csv',index=False)
    print("\nSaved to tmp/moving_m15_h1_results.csv")
