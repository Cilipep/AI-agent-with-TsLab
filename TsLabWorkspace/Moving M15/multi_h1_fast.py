#!/usr/bin/env python3
"""Multi-instrument backtest on H1 - fast version"""
...
def main():
    print("="*80)
    print("MULTI-INSTRUMENT — H1 TIMEFRAME")
    print("="*80)
    ...
    for sym in INSTRUMENTS:
        ...
        best=None
        for sl,tp,sp,wp in product([0.03],[0.06,0.08],[7],[10]):
            if tp<=sl: continue
    ...
    print("\n"+"="*80)
    print("H1 RESULTS (SL=3%, SMMA=7, WR=10)")
    print("="*80)
    print(f"{'Sym':<7} {'TP%':>4} {'#':>3} {'WR%':>5} {'Ret%':>7} {'MDD%':>6} {'AW%':>5} {'AL%':>5}")
    print("-"*80)
    for r in results:
        s="+" if r['ret']>0 else ""
        print(f"{r['sym']:<7} {r['tp']:.0f} {r['trades']:>3} {r['wr']:>5.1f} {s}{r['ret']:>6.2f} {r['mdd']:>6.1f} {r['aw']:>5.2f} {r['al']:>5.2f}")
    ...
    print("Saved to tmp/moving_m15_h1_results.csv")
