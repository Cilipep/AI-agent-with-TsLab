#!/usr/bin/env python3
"""Multi-instrument backtest on M15 timeframe"""

import pandas as pd
import numpy as np
from itertools import product

INSTRUMENTS = ["SOL","LINK","INJ","ETH","DOGE","FET","BTC"]
...
def main():
    print("="*80)
    print("MULTI-INSTRUMENT — M15 TIMEFRAME")
    print("="*80)
    ...
    print("\n"+"="*80)
    print("M15 RESULTS (sorted by return)")
    print("="*80)
    print(f"{'Sym':<7} {'SL':>3} {'TP':>3} {'S':>2} {'WR':>2} {'#':>3} {'WR%':>5} {'Ret%':>7} {'MDD%':>6} {'AW%':>5} {'AL%':>5}")
    print("-"*80)
    for r in results:
        s="+" if r['ret']>0 else ""
    ...
    pd.DataFrame(results).to_csv('tmp/moving_m15_m15_results.csv',index=False)
    print("Saved to tmp/moving_m15_m15_results.csv")
