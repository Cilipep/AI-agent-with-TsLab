#!/usr/bin/env python3
"""Continue M15 analysis for remaining instruments"""

import pandas as pd
import numpy as np
from itertools import product

INSTRUMENTS = ["ETH","DOGE","FET","BTC"]
...
def dl(sym):
    ...
    for f in [f"{sym}-USD", f"{sym}USD"]:
        try:
            t=yf.Ticker(f); d=t.history(period="60d", interval="15m")
...
def main():
    ...
    for sym in INSTRUMENTS:
        ...
        best=None
        for sl,tp,sp,wp in product([0.02,0.03],[0.04,0.06,0.08],[5,7],[10,14]):
            if tp<=sl: continue
    ...
    earlier=[{'sym':'SOL','sl':2,'tp':4,'smma':7,'wrp':10,'trades':19,'wr':28,'ret':6.68,'mdd':12.7,'aw':3.80,'al':-1.56},
             {'sym':'LINK','sl':3,'tp':8,'smma':5,'wrp':10,'trades':18,'wr':48,'ret':22.73,'mdd':5.8,'aw':7.17,'al':-2.61},
             {'sym':'INJ','sl':2,'tp':4,'smma':5,'wrp':14,'trades':23,'wr':24,'ret':2.20,'mdd':15.6,'aw':3.80,'al':-1.88}]
    all_res=earlier+results
    all_res.sort(key=lambda x: x['ret'], reverse=True)
    
    print("\n"+"="*80)
    print("M15 COMPLETE RESULTS")
    print("="*80)
    ...
    prof=[r for r in all_res if r['ret']>0]
    print("-"*80)
    print(f"Profitable: {len(prof)}/{len(all_res)} | Avg Return: {np.mean([r['ret'] for r in all_res]):.2f}%")
