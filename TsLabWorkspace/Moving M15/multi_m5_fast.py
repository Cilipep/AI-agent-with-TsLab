#!/usr/bin/env python3
"""M5 analysis - simplified, one instrument at a time"""
...
def dl(sym):
    ...
    for f in [f"{sym}-USD", f"{sym}USD"]:
        try:
            t=yf.Ticker(f); d=t.history(period="1mo", interval="5m")
...
for sym in ["SOL","LINK","ETH","DOGE","FET"]:
    print(f"{sym}...", end=" ", flush=True)
    df=dl(sym)
    if df is None or df.empty: print("SKIP"); continue
    print(f"{len(df)} bars", end=" ")
    best=None
    for sl,tp,sp,wp in [(0.01,0.02,3,8),(0.01,0.04,3,8),(0.02,0.04,5,10),(0.02,0.06,5,10),(0.01,0.02,5,10)]:
