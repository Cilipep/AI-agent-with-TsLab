#!/usr/bin/env python3
"""Optimization: Moving M15 - SOL on 1H timeframe"""
...
def main():
    print("="*80)
    print("OPTIMIZATION: Moving M15 - SOL (1H timeframe)")
    print("="*80)
    df=dl("SOL")
    ...
    results=[]
    for sl,tp,sp,wp in product([0.02,0.03,0.04,0.05],[0.04,0.06,0.08,0.10],[5,7,10],[10,14]):
        if tp<=sl: continue
    ...
    pd.DataFrame(results).to_csv('tmp/moving_m15_opt_sol_1h.csv',index=False)
    print("Saved to tmp/moving_m15_opt_sol_1h.csv")
