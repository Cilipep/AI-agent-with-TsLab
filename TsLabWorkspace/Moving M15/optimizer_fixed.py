#!/usr/bin/env python3
"""
Moving M15 Strategy - Fixed Optimizer
Uses correct symbol formats and shorter periods
"""
...
def main():
    print("=" * 80)
    print("MOVING M15 STRATEGY - OPTIMIZER (fixed)")
    print("=" * 80)
    
    instruments = ['SOL', 'DOGE', 'LINK', 'ETH', 'FET', 'BTC']
    all_results = []
    
    # Test on 1H with 3mo period
    ...
    if all_results:
        ...
        pd.DataFrame(all_results).to_csv('tmp/moving_m15_optimized.csv', index=False)
        print("\nSaved to tmp/moving_m15_optimized.csv")
