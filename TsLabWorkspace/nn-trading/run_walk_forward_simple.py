"""Run walk-forward validation with simple optimized parameters."""
import os
import json
import numpy as np
import pandas as pd
...
def main():
    ...
    print("=" * 60)
    print("WALK-FORWARD VALIDATION (SIMPLE)")
    print("=" * 60)
    print(f"\nUsing simple optimized params from: {params_path}")
    ...
    equity, results = walk_forward(df, cfg, device, n_folds=6)

    # Save results
    output_path = cfg.result_dir / "walk_forward_simple_results.json"
    output_data = {
        "params": params,
        "n_folds": len(results),
