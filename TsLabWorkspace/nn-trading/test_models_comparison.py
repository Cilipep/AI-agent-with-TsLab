"""Compare LSTM, TCN, and Transformer models."""
import os
import json
import random
import numpy as np
...
def train_and_test(model_type, df, cfg, ds, device="cpu"):
    """Train ensemble and test with enhanced backtest."""
...
def main():
    ...
    print("\n" + "=" * 70)
    print("MODEL COMPARISON (Enhanced Backtest)")
    ...
    print(f"\n{'Model':<15} {'Return':>10} {'Drawdown':>10} {'Win Rate':>10} {'PF':>8}")
    print("-" * 55)

    for model_type, result in results.items():
        print(f"{model_type.upper():<15} {result['total_return_pct']:>+9.2f}% "
    ...
    with open(cfg.result_dir / "models_comparison.json", "w") as f:
        json.dump(output, f, indent=2)

    print(f"\nResults saved to {cfg.result_dir / 'models_comparison.json'}")
