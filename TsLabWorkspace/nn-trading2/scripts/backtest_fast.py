"""Fast backtest with reduced features and models."""
import warnings
...
def main():
    ...
    cfg.num_layers = 2
    cfg.dropout = 0.35
    cfg.window = 20  # Reduced from 30
    cfg.batch_size = 64
    cfg.learning_rate = 0.001
    cfg.n_models = 2  # Reduced from 3
    cfg.max_features = 20  # Reduced from 40
    cfg.stop_loss_pct = 0.03
    cfg.take_profit_pct = 0.07
    cfg.n_cpu_threads = 2
    cfg.epochs = 20  # Reduced from 30

    print("=" * 70)
    print("FAST BACKTEST (Ensemble + Walk-Forward)")
    print("=" * 70)
    ...
    with open("results/fast_backtest_ensemble.json", "w") as f:
        json.dump(output, f, indent=2)
    with open("results/equity_curves_fast.json", "w") as f:
        json.dump({r["instrument"]: r["equity_curve"] for r in all_results} | {"portfolio": portfolio.get("equity_curve",[])}, f)
    print(f"\nSaved to results/fast_backtest_ensemble.json")
