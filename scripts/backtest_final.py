...
def backtest_final(df, capital=25, leverage=1.7, max_drawdown_limit=0.25):
    ...
    # Оптимальные параметры (снижена просадка)
    risk_pct = 0.003
    sl_mult = 1.6
    tp_mult = 2.5
    trail_pct = 0.008
    ...
    # Адаптивный реинвест: 8/2/0
    reinvest_config = {1: 0.08, 0: 0.02, -1: 0.00}
...
def main():
    ...
    print(f"  Stop Loss:      1.6x ATR")
    print(f"  Take Profit:    2.5x ATR")
    print(f"  Trail Stop:     0.8%")
    print(f"  Реинвест:       8% (↑) / 2% (→) / 0% (↓)")
    ...
    # Бэктест
    result = backtest_final(df, capital=25, leverage=2, max_drawdown_limit=0.25)
